(function ($) {
  $(function () {
    ...
    $('.jc-export-products').on('click', function (event) {
      ...
      const $btn = $(this).addClass('disabled').attr('aria-disabled', 'true').text('正在导出…');
      ...
      $.post(window.jc_ajax.ajax_url, {
        ...
        .done(function (response) {
          if (response && response.success && response.data && response.data.url) {
            ...
            window.alert(response.data && response.data.message ? response.data.message : '导出失败。');
          }
        })
        .fail(function () {
        ...
        .always(function () {
          $btn.removeClass('disabled').removeAttr('aria-disabled').text('导出产品');
        });
    });
    ...
    $fileInput.on('change', function () {
      ...
      const $btn = $('.jc-import-products').addClass('disabled').attr('aria-disabled', 'true').text('正在导入…');
      ...
      $.ajax({
        ...
        .done(function (response) {
          if (response && response.success) {
            window.alert('导入完成，新增：' + (response.data.imported || 0) + '，更新：' + (response.data.updated || 0));
            window.location.reload();
          } else {
            window.alert(response.data && response.data.message ? response.data.message : '导入失败。');
          }
        })
        .fail(function () {
        ...
        .always(function () {
          $btn.removeClass('disabled').removeAttr('aria-disabled').text('导入产品');
          $fileInput.val('');
        });
    });
  });
