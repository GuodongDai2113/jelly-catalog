(function ($) {
  $(function () {
    ...
    function showProgress(message) {
      ...
      renderProgressBar(
        currentPercent,
        $progress.hasClass("jc-port-progress--error") ? "error" : "default",
      );
    }
    ...
    function failImport(message, canResume) {
      ...
      $message.text(
        (message || settings.messages.failed) +
          (canResume ? " " + settings.messages.resumeReady : ""),
      );
    ...
    function processNextBatch(retryCount) {
      ...
      $.post(settings.ajaxUrl, {
        ...
        .done(function (response) {
          if (!response || !response.success) {
            ...
            var message = getResponseMessage(
              response,
              settings.messages.failed,
            );

            if (
              retryable &&
              scheduleRetry(processNextBatch, retryCount, message)
            ) {
              return;
            }
          ...
          if (response.data.status === "waiting") {
            ...
            if (lockWaits > maxLockWaits) {
              failImport(
                settings.messages.lockTimeout || settings.messages.failed,
                true,
              );
              return;
            }
        ...
        .fail(function (xhr) {
          ...
          var message = getResponseMessage(
            response,
            settings.messages.networkError,
          );
      ...
      $.ajax({
        ...
        .done(function (response) {
          ...
          if (response.data.status === "complete") {
            ...
            renderProgressBar(100, "success");
            jellyCore.showMessage(
              response.data.message || settings.messages.complete,
              "success",
            );
            return;
          }
        ...
        .fail(function (xhr) {
          ...
          if (
            ...
            scheduleRetry(
              ...
              retryCount,
              message || settings.messages.networkError,
            )
          ) {
            return;
          }
    ...
    $form.on("submit", function (event) {
      ...
      var hasProductFile = !!(
        productFile &&
        productFile.files &&
        productFile.files.length
      );
      var hasCategoryFile = !!(
        categoryFile &&
        categoryFile.files &&
        categoryFile.files.length
      );

      if (!hasProductFile && !hasCategoryFile) {
        jellyCore.showMessage(
          settings.messages.missingFiles ||
            "Please upload a product CSV or a category CSV.",
          "warning",
          5000,
        );
        return;
