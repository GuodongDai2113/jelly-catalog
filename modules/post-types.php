...
class Post_Types {
	...
	public function __construct() {
		...
		add_action( 'pre_get_posts', array( $this, 'product_query' ), 9 );
		add_filter( 'terms_clauses', array( $this, 'product_cat_terms_clauses' ), 10, 3 );
	}
	...
	public function register_taxonomies() {
		...
		register_taxonomy(
			...
			array(
				...
				'show_ui'      => true,
				'query_var'    => true,
				'rewrite'      => $category_rewrite_slug ? array(
	...
	/**
	 * 前端 product_cat 查询排序，依据 settings 配置。
	 *
	 ...
	 * @return array
	 */
	public function product_cat_terms_clauses( $clauses, $taxonomies, $args ) {
		if ( is_admin() || ! in_array( 'product_cat', (array) $taxonomies, true ) ) {
			return $clauses;
		}

		$orderby = get_option( 'products_cat_orderby', 'term_order' );
		$order   = strtoupper( get_option( 'products_cat_order', 'ASC' ) );
		...
		$orderby_sql = isset( $orderby_map[ $orderby ] ) ? $orderby_map[ $orderby ] : 't.term_order';
		$order_sql   = in_array( $order, array( 'ASC', 'DESC' ), true ) ? $order : 'ASC';

		$clauses['orderby'] = "ORDER BY tt.parent ASC, {$orderby_sql} {$order_sql}";
		$clauses['order']   = '';
		return $clauses;
	}
}
